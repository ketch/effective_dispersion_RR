...
#E2=5./2;   p2=2./5
r=8;
p1=r+np.sqrt(r**2-r); E1=1
p2=r/p1; E2=1
# interface parameters
alphax=0.5; deltax=1000.0
alphay=0.5; deltay=1.0
# Linearity parameters
linearity_mat1=1; linearity_mat2=1
# heterogeneity type
...
def compute_p(state):
    K=state.aux[1,:,:]
    linearity=state.aux[2,:,:]
    eps=state.q[0,:,:]
    state.p[0,:,:]=np.where(linearity==1,1,0)*K*eps+np.where(linearity==2,1,0)*(np.exp(eps*K)-1)
...
def psystem2D(iplot=False,kernel_language='Fortran',htmlplot=False,
    ...
    ####################################
    # Domain
    x_lower=0.0; x_upper=40.00
    y_lower=0.0; y_upper=20.0
    # cells per layer
    Nx=16#32
    Ny=64#128
    ...
    bc_x_lower=pyclaw.BC.extrap; bc_x_upper=pyclaw.BC.extrap
    bc_y_lower=pyclaw.BC.periodic; bc_y_upper=pyclaw.BC.periodic
