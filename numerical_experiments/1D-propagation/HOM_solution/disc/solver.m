...
title(['Discontinuous domain at t=' num2str(t)],'fontsize',20)
...
plot(x,s6,'b')
title(['Discontinuous domain at t=' num2str(t)],'fontsize',20)
axis([62+xshift 65+xshift -0.075 0.075])
%% plot finite volume solution
FV=textread('stress.txt');
xFV=FV(:,1); xFV=linspace(xshift,200+xshift,length(xFV))+xFV(1); 
sFV=FV(:,2);
plot(xFV,sFV,'--k','linewidth',1)
leg=legend('leading order','2nd correction','4th correction','6th correction','FV solution','location','northwest');
set(leg,'FontSize',20)
...
% adjust the effective sound speed depending on the type of media
ceff=sqrt(Kh/ph);

% physical domain
x_lower=0; x_upper=400;
mx=2^12; %Number of Fourier modes
Lx=x_upper-x_lower;
