...
def plot_p(frame,slices_xlimits=None,init_cond=False):
    ...
    pl.savefig('./_plots_to_paper/c-dispersion_sinusoidal_pcolor_frame'+str_frame+'.png')
    pl.close()

    # write slices
    fx=open('c-dispersion_sinusoidal_xslice.txt','w')
    fx.writelines(str(xc)+" "+str(p[i,0])+"\n" for i,xc in enumerate(x))
    fx.close()

    fy=open('c-dispersion_sinusoidal_yslice.txt','w')
    ...
    #pl.plot(x,p[:,0],'b')
    #pl.plot(y,p[0,:],'r')
    #pl.savefig('./_plots_to_paper/c-dispersion_sinusoidal_frame'+str_frame+'_slices.png')
    #pl.close()
...
if __name__== "__main__":
    ...
    #frames=[0,10,20,30,40,50,60,70,75,80,90,100]
