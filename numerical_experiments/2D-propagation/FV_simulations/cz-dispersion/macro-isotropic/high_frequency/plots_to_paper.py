...
def plot_p(frame,slices_xlimits=None,init_cond=False):
    ...
    pl.savefig('./_plots_to_paper/cz-dispersion_macro-iso_high-freq_pcolor_frame'+str_frame+'.png')
    pl.close()

    # write slices
    fx=open('cz-dispersion_macro-iso_high-freq_layered_xslice.txt','w')
    fx.writelines(str(xc)+" "+str(p[i,0])+"\n" for i,xc in enumerate(x))
    fx.close()

    fy=open('cz-dispersion_macro-iso_high-freq_layered_yslice.txt','w')
    ...
    #pl.plot(x,p[:,0],'b')
    #pl.plot(y,p[0,:],'r')
    #pl.savefig('./_plots_to_paper/cz-dispersion_macro-iso_high-freq_frame'+str_frame+'_slices.png')
    #pl.close()
