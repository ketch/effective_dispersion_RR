...
for i,p in enumerate([p,pc,pz,pcz]):
    ...
    if on_top[i] == 1:
        ...
        axx[-1].set_xticklabels(axx[-1].get_xticks(),ticks_font,color=textcolor)
    ...
    if on_right[i] == -1:  # Tick labels on left
        ...
        axy[-1].set_yticklabels(axy[-1].get_yticks(),ticks_font,color=textcolor)
    else:
    ...
    #    plt.clim(0.9,1.1)
    if i == 1:
        axd[-1].text(0.5,1.02,'$k_x$',color=textcolor,fontsize=15*multiplier,transform=axd[-1].transAxes,verticalalignment='bottom',horizontalalignment='center')
        axd[-1].text(1.08,0.5,'$k_y$',color=textcolor,fontsize=15*multiplier,transform=axd[-1].transAxes,verticalalignment='center',horizontalalignment='left')
    elif i == 3:
        ...
        cbar.outline.set_color(textcolor)
        cbar.ax.set_yticklabels([])
        #cbar.set_label('sound speed',color=textcolortsize=10)
